...
- [x] Extend safe traces with per-state duration, provider/model identity, fallback usage, provider-call count, retrieval counts/scores, repair count, and validation category.
- [x] Evaluate a datasource/hash-scoped retrieval-context cache against Phase 10 evidence and defer implementation: the suite measures independent questions and provides no repeated-question hit-rate or material embedding-cost evidence. Any future cache must invalidate by metadata hash, profile hash, and retrieval-config version rather than TTL alone.
